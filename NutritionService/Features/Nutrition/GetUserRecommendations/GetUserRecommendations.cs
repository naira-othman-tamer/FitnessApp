...
public sealed class GetUserRecommendationsHandler(IUnitOfWork<NutritionDbContext> unitOfWork, IFceClient fce, IClaimsManager claims)
    : IRequestHandler<GetUserRecommendationsQuery, OperationResult<UserRecommendationDto>>
{
    public async Task<OperationResult<UserRecommendationDto>> Handle(GetUserRecommendationsQuery request, CancellationToken cancellationToken)
    {
        ...
        var plans = await unitOfWork.Repository<MealPlan, Guid>().Query().AsSplitQuery().Include(x => x.Items).ThenInclude(x => x.Meal).ThenInclude(x => x.Tags)
