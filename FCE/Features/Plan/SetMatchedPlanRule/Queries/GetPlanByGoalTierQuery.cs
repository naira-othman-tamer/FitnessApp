...
{
    public record GetPlanByGoalTierQuery(Goal goal, CalorieIntensityTier tier) : IRequest<int>;

    public class GetPlanByGoalTierQueryHandler : IRequestHandler<GetPlanByGoalTierQuery, int>
    {
        private readonly GeneralRepository<PlanRule> _planRuleRepo;

        public GetPlanByGoalTierQueryHandler(GeneralRepository<PlanRule> planRuleRepo)
        {
        ...
        public async Task<int> Handle(GetPlanByGoalTierQuery request, CancellationToken cancellationToken)
        {
            var externalPlanRuleId = await _planRuleRepo
                .Get(p=>p.calorieIntensityTier==request.tier && p.goal== request.goal)
